...
export default function MyPPTPage() {
  ...
  const fetchList = async () => {
    ...
    setLoading(true);
    try {
      const data = await request(`${API}?type=ppt&mine=1`);
      setList(data);
    } catch (e) {}
    setLoading(false);
  };
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 32 }}>
      <Title level={3}>我的PPT</Title>
      <Button icon={<UploadOutlined />} type="primary" onClick={handleUpload} style={{ marginBottom: 16 }}>新建PPT</Button>
      <List
            ...
            <Card
              ...
              extra={<>
                <Button icon={<EyeOutlined />} size="small" onClick={() => {
                  if (item.pdf_cover) {
                    setPreview(BASE_URL + item.pdf_cover);
                  } else if (item.cover) {
                    setPreview(BASE_URL + item.cover);
                  } else {
                    message.warning('无可预览文件');
                  }
                }}>预览</Button>
                <Button
      ...
      />
      <ModalPPTPreview url={preview} onClose={() => setPreview(null)} />
      <Modal
        open={modalOpen}
        title={editItem ? '编辑PPT' : '新建PPT'}
      ...
      >
        ...
        <div style={{ marginTop: 16 }}>
          <div style={{ fontWeight: 500, marginBottom: 8 }}>上传PPT/PDF文件 <span style={{ color: 'red' }}>*</span></div>
          <Upload beforeUpload={f => { setFile(f); return false; }} maxCount={1} accept=".ppt,.pptx,.pdf" listType="text" fileList={file ? [{ uid: '-1', name: file.name }] : []} onRemove={() => setFile(null)}>
...
function ModalPPTPreview({ url, onClose }: { url: string | null, onClose: () => void }) {
