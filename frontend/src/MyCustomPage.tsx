...
export default function MyCustomPage() {
  ...
  const fetchList = async () => {
    ...
    setLoading(true);
    try {
      const data = await request(`${API}?type=custom&mine=1`);
      setList(data);
    } catch (e) {}
    setLoading(false);
  };
  ...
  const handleModalOk = async () => {
    try {
      ...
      const result = await request(editItem ? `${API}/${editItem.id}` : API, {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 32 }}>
      <Title level={3}>我的定制</Title>
      <Button icon={<UploadOutlined />} type="primary" onClick={handleUpload} style={{ marginBottom: 16 }}>新建定制</Button>
      <List
      ...
      />
      <ModalCustomPreview url={preview} onClose={() => setPreview(null)} />
      <Modal
        open={modalOpen}
        title={editItem ? '编辑定制' : '新建定制'}
      ...
      >
        ...
        <div style={{ marginTop: 16 }}>
          <div style={{ fontWeight: 500, marginBottom: 8 }}>上传图片/音频文件 <span style={{ color: 'red' }}>*</span></div>
          <Upload beforeUpload={f => { setFile(f); return false; }} maxCount={1} accept=".jpg,.png,.mp3,.wav" listType="text" fileList={file ? [{ uid: '-1', name: file.name }] : []} onRemove={() => setFile(null)}>
...
function ModalCustomPreview({ url, onClose }: { url: string | null, onClose: () => void }) {
